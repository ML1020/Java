...
public class Hill {
    ...
    public static void encry(int n ,String text) {
        ...
        for(int i=0;i<n;i++) {
            list.add(new Hill(n).a);//用对象的属性（数组）作为arraylist的元素
        }
        System.out.println(list);//输出加密数组
//System.out.println(l.get(1).get(0)); 测试具体值
        ...
        for(int j= 0;j<n;j++) {//几个字母几次
            for(int k =0;k<n;k++) {//数组个数
                int sum = 0;
                for(int i = 0;i<list.get(k).size();i++) {//数组里的元素个数
                    sum+=((int)list.get(k).get(i))*origin[i];//矩阵相乘
                }
                result[k]=sum%26;
            }
        }
